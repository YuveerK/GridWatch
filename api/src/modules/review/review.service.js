...
import { checkPostDispositions } from '../processing/quality.js';
...
export async function detectSuspicious({ prisma, postIds, now = new Date() }) {
  ...
  const posts = await prisma.sourcePost.findMany({
    where: { id: { in: postIds } },
    select: {
      id: true, processingStatus: true,
      extractions: { where: { promptVersion: env.AI_PROMPT_VERSION, status: 'SUCCEEDED' }, orderBy: { createdAt: 'desc' }, take: 1 },
  ...
  for (const x of posts) {
    ...
    if (x.processingStatus === 'NEEDS_REVIEW' || (result && (result.confidence < 0.75 || result.review_reason))) add(x.id, 0, 'UNCERTAIN_READING', result?.review_reason ? `the reader said: ${String(result.review_reason).slice(0, 100)}` : `confidence ${result?.confidence ?? 'n/a'}`);
    ...
    if (e?.result) {
      const items = faultItems({ ...e, result: e.result });
      ...
      if (linkable) for (const d of verdict.excluded) add(x.id, d.faultIndex, 'DISCARDED_FAULT', d.reason);
    }
...
export async function runReview({ prisma, postIds, now = new Date(), generate, maxVerifications = 5 }) {
  ...
  return { flagged: suspicions.length, opened, verified };
}

export async function listReviewItems({ prisma, status = 'OPEN', limit = 50 }) {
  const rows = await prisma.reviewItem.findMany({
    where: { status },
    orderBy: [{ priority: 'desc' }, { createdAt: 'asc' }],
    take: limit,
    include: { post: { select: { externalId: true, publishedAt: true, text: true, noteTweetText: true } } },
  });
  return rows.map((r) => ({ id: r.id, postId: r.postId, externalId: r.post.externalId, publishedAt: r.post.publishedAt, faultIndex: r.faultIndex, priority: r.priority, reasons: r.reasons, verifier: r.verifier, sampled: r.sampled, text: (r.post.noteTweetText || r.post.text || '').replace(/\s+/g, ' ').slice(0, 220) }));
}
